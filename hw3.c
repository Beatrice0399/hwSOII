...
void justify(int *nwords, int* row, char** array, char **outputText, int* countColumn, int* distanceColumn, int* nColumn, int* countRow,
    int* linesPerColumn, int* countPage, int* columnWidth, int* total_length, int* nPage);
void writeText(char** array);
void inizializza(char** array, int* size, int* length);
void noJustified(int *nwords, int* row, char** array, char** outputText, int* columnWidt, int conSpazi, int* distanceColumn, int* ncolumn, 
    int* countColumn, int* linesPerColumn, int* countRow, int* countPage, int* total_length, int* nPage);
void printArray(char** a, int* linesPerColumn, int* countPage);
void emptyRow(int* row, char** outputText, int* columnWidth, int* distanceColumn, int* ncolumn, int* countColumn, int* countRow,
     int*linesPerColumn, int* countPage, int* total_length, int* nPage);
void newPage(char** outputText, int* row, int*countPage, int* linesPerColumn, int* total_length, int*nPage); 
    ...
    for (char* token = strtok(input_text, " "); token != NULL; token = strtok(NULL, " ")) {
        ...
        if (occurrence) {
            ...
            if ( (conspazi+lenToken1) <= *columnWidth) {    //<=
                ...
                *nwords += 1;
                // NO JUSTIFY
                noJustified(nwords, row, array, outputText, columnWidth, conspazi, distanceColumn, nColumn, countColumn, linesPerColumn, countRow, countPage, pageLength, nPage);
                //*nwords = 1;
                //strcpy(array[0], token2);
                //conspazi = len(token2) +1;
                if (*row != *countPage) {
                    emptyRow(row, outputText, columnWidth, distanceColumn, nColumn, countColumn, countRow, linesPerColumn, countPage, pageLength, nPage);
                }
                ...
                linesPerColumn, countPage, columnWidth, pageLength, nPage);
                *nwords = 0;
                strcpy(array[*nwords], token1);
                *nwords +=1;
            ...
            *nwords += 1;
            ...
            justify(nwords, row, array, outputText, countColumn, distanceColumn, nColumn,
                                    countRow, linesPerColumn, countPage, columnWidth, pageLength, nPage);
    ...
    *countRow += 1;
    ...
    if (*countRow == *linesPerColumn ) { //-1
        if (*countColumn == *nColumn-1) {
            ...
            //*countPage = *row + 3;
            //REALLOC
            newPage(outputText, row, countPage, linesPerColumn, total_length, nPage);
            *countRow = 0;
            *countColumn = 0;
        }
        else {
...
void noJustified(int *nwords, int* row, char** array, char** outputText, int* columnWidth, int conSpazi, int* distanceColumn, int* ncolumn, int* countColumn, 
            int* linesPerColumn, int* countRow, int* countPage, int* total_length, int* nPage){
    int count = 0;
    int lenParole = 0;
    for (int i = 0; i < *nwords; i++) {
...
void emptyRow(int* row, char** outputText, int* columnWidth, int* distanceColumn, int* ncolumn, int* countColumn, int* countRow, int*linesPerColumn, int* countPage, int* total_length, int* nPage){
    char* str = malloc(100* sizeof(*str));
